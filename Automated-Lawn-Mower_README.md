...
2. Install the **NewPing** and **Servo** libraries via Library Manager (Servo ships with the IDE by default).
3. Wire up the motors, H-bridge, ultrasonic sensor, and servo per the pin table above.
4. Open `Lawn_Mower.ino`, select your board and port, and upload.
5. Power the motor driver from the battery pack (not the Arduino's 5V rail) to avoid brownouts when the motors draw current.
